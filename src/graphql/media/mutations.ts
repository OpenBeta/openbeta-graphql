...
const MediaMutations = {
  addMediaObjects: async (_: any, args, { dataSources, user }: ContextWithAuth): Promise<MediaObject[]> => {
    const { media } = dataSources
    const { input }: { input: MediaObjectGQLInput[] } = args
    return await media.addMediaObjects(input)
  },

  deleteMediaObject: async (_: any, args, { dataSources }: ContextWithAuth): Promise<boolean> => {
    const { media } = dataSources
    const { input }: { input: DeleteMediaGQLInput } = args
    return await media.deleteMediaObject(new mongoose.Types.ObjectId(input.mediaId))
  },

  addEntityTag: async (_: any, args, { dataSources }: ContextWithAuth): Promise<EntityTag> => {
    const { media } = dataSources
  ...
  removeEntityTag: async (_: any, args, { dataSources }: ContextWithAuth): Promise<boolean> => {
    const { media } = dataSources
